...
//handle submit button
  $('#user_register_form').on('submit', function(event){
    ...
    {
      $.ajax({
				//if valid use ajax to send to database
        url:'user_ajax_action.php',
        method:"POST",
        ...
        beforeSend:function()
        {
					//disable sumbit to prevnet double input
        ...
        success:function(data)
        {
          if(data.success)
          {
						//if success send messsage
            $('#message').html('<div class="alert alert-success">check your email</div>');
